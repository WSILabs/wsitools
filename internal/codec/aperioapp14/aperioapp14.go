...
// wsi_encode_aperio encodes width*height 8-bit RGB pixels into a JPEG stream.
//
...
// New creates an Encoder for the given level geometry and quality.
...
func (e *Encoder) computeTables() error {
	...
	if err != nil {
		return fmt.Errorf("codec/aperioapp14: probe encode: %w", err)
	}
	tables, err := tiff.ExtractJPEGTables(full)
	if err != nil {
		return fmt.Errorf("codec/aperioapp14: extract tables: %w", err)
	}
	e.tables = tables
	return nil
}
...
func (e *Encoder) EncodeTile(rgb []byte, w, h int, dst []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("codec/aperioapp14: EncodeTile: %w", err)
	}
...
// encodeRaw calls the C wsi_encode_aperio helper which drives the libjpeg compress loop.
//
...
// inside wsi_encode_aperio; we only cross the cgo boundary with a plain data pointer
...
func (e *Encoder) encodeRaw(rgb []byte, w, h int, abbreviated bool) ([]byte, error) {
	...
	if ret != 0 {
		return nil, fmt.Errorf("codec/aperioapp14: wsi_encode_aperio returned %d", ret)
	}
	if outBuf == nil {
		return nil, fmt.Errorf("codec/aperioapp14: wsi_encode_aperio produced nil output")
	}
