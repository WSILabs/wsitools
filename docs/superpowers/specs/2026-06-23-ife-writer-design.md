...
**PNG associated read-back (resolved in opentile-go v0.49.0).** Non-JPEG/AVIF
associated images (e.g. an Aperio LZW label) are stored as lossless **PNG**
(`encoding=1`) so the label stays lossless (crisp barcodes). This is
spec-conformant (the official Iris-Codec validator accepts it). Originally
opentile-go mapped `IMAGE_ENTRY.encoding==1` to `CompressionUnknown` and could not
decode it — filed as **opentile-go#74** and **fixed in v0.49.0** (PNG associated
decode via a pure-Go `image/png` branch). wsitools maps `opentile.CompressionPNG`
→ `source.CompressionPNG` so `info` shows `png` and `extract --type label` decodes
the PNG label. JPEG/AVIF associated images round-trip on any version.
