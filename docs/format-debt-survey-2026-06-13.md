...
| C4 | ~~**`associated replace` on SVS works only for `label`** (thumbnail/macro/overview rejected — abbreviated-JPEG reconstruction).~~ **DONE** (merge TBD) — the survey's "abbreviated-JPEG" reason was wrong: opentile reads a standalone single-strip JPEG fine (`decodeStripStack` fallback). Real blocker is IFD position. `label`/`macro`/`overview` trail the tiled pyramid → splice tail-rewrite works (pyramid byte-identical). `thumbnail` works too when no tiled level follows it (single-level slides); on multi-level slides it can't relocate the tiled levels and errors clearly (`ErrUnexpectedLayout`→`ErrUnsupportedAssoc`). Blanket gate removed; honest error retained. Deferred: thumbnail rebuild on multi-level (re-stream pyramid). | `cmd/wsitools/associated.go` | [confirmed] | S | Low |
