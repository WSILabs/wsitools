...
| C3 | ~~**`native.go` force-overrides PixelData VR `OW→OB`** to work around a `suyashkumar/dicom` hardcode — brittle; breaks to grayscale silently if upstream changes.~~ **DONE** (merge 8495c31) — fixed at the source: `WSILabs/dicom` v1.1.0-wsilabs.2 `pixelDataVR` selects `OB` for 8-bit native PixelData (else `OW`); the manual override is removed. Root cause was a pre-existing upstream bug (`suyashkumar/dicom#299`, only half-fixed by #316: the tag dict lists OW+OB but `NewElement` never chose between them). Guarded by fork `TestNewElement_PixelDataVR` + existing `TestNativePixelDataRoundTrip`. | `internal/dicomwriter/native.go` | [confirmed] | S | Low–Med |
