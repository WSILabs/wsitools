...
| A4a | **`convert --to dicom` frame-copies JPEG/JP2K only, but HTJ2K and JPEG XL *are* DICOM transfer syntaxes** — so they should be **verbatim frame-copied with the correct TransferSyntax UID** (lossless, like JPEG/JP2K today), NOT decode→re-encoded. HTJ2K = `…1.2.4.201`/`.202`/`.203` (Sup 232); JPEG XL = `…1.2.4.110`/`.111`/`.112` (Sup 235). Needs a TS-UID branch + a codestream inspector (`InspectHTJ2K`/`InspectJXL`, analogous to `InspectJP2K`) to derive photometric/reversibility. **HTJ2K half = B4** (see there). Caveats: DICOM constrains the encapsulated form (HTJ2K wants the raw `.j2c` codestream, not `.jph`-boxed; JXL its codestream form) — confirm source frames match; and `dciodvfy`/downstream readers may not yet recognize these newer syntaxes, so "conformant" is harder to claim. The F1 `DecodedTile` seam is **not** needed here (no decode). | `dicomwriter.go:398`, `:351` | [confirmed] | M (per codec) | Med |
