...
| # | Item | Where | Conf | Effort | Impact |
|---|---|---|---|---|---|
| B1 | **JPEG 2000 encoder: BUILT + verified, PARKED** (branch `feat/b1-jp2k-encoder`, pushed, **not merged**) — `internal/codec/jp2k` (OpenJPEG cgo, mirrors htj2k; `q` + `reversible` knobs; raw J2K codestream). Verified byte-exact via OpenJPEG's own `opj_decompress`. **Blocked on opentile-go#53**: opentile's JP2K decoder force-assumes 3-comp raw codestreams are YCbCr, so it mis-renders our RGB output → can't ship `--codec jpeg2000` (own reader shows wrong colors) nor verify the JP2K-lossless DICOM path in-repo. Merge once #53 lands (then wire `--codec jpeg2000` + flip the byte-exact round-trip test from skip to assert). | `internal/codec/jp2k/` | [confirmed firsthand] | M–L | Low–Med |
