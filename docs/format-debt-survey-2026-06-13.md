...
| D8 | ~~**dciodvfy = dclunie single point of failure for CI**~~ **DONE** (branch `ci/d8-d9-dciodvfy-mirror`) — the pinned dicom3tools macexe (BSD; redistribution OK with attribution) is mirrored on `wsilabs/wsi-fixtures` release **`tools-v1`** (+ `dicom3tools-COPYRIGHT.txt`); CI's Download-dciodvfy step now `gh release download`s from there instead of dclunie.com, so a dclunie move/outage no longer breaks CI. | `ci.yml`, `wsi-fixtures tools-v1` | [confirmed firsthand] | S | Med |
