...
| D3 | **No JP2K-SVS / OME-TIFF / Leica-SCN / generic-TIFF CI fixtures** → those paths skip in CI. **PARTIAL** (merge bf5c81f): wsi-fixtures **v7** added an OME-TIFF (`CMU-1-Small-Region.ome.tiff` → OME-TIFF transform CI coverage) + the `590_crop` ImageScope crops (JP2K-SVS + LZW/uncompressed TIFF). Still open: Leica-SCN. **Multi-level SVS fixture identified** (`239551.svs`, owner's own scan, 25 MB, JPEG-tiled, 3 levels + thumbnail/label/overview) — the only in-CI SVS fixtures are single-level, so the `convert --to svs` multi-level thumbnail/overview path and the `rebuildSVS` integration are CI-uncovered. **OWNER ACTION to land it:** add `239551.svs` to the wsi-fixtures `svs.tar`, cut release **v8**, then in `.github/workflows/ci.yml` bump `gh release download v7`→`v8` and cache key `wsi-fixtures-v7`→`v8`, and add to `.github/fixtures.sha256`: `019946dbe04eefaee18a90bb3222ae5ac51f081736ff1e195d0a52169ccc9535  svs/239551.svs`. (Do NOT add the sha line before the tar ships — CI does a strict `shasum -c` and would fail on the missing file. Until then the new tests skip-if-absent in CI.) Note: 239551 happens to splice on thumbnail replace, so CI coverage of the `rebuildSVS` *path* specifically would additionally want a CMU-1-style multi-level slide whose L0 data interleaves the thumbnail offsets. | fixtures.sha256 | [confirmed] | M | Med |
